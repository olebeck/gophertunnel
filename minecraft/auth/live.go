...
func (conf Config) RequestLiveTokenWriter(w io.Writer) (*oauth2.Token, error) {
	return conf.RequestLiveTokenContext(context.Background(), CliAuthCodeHandler{Writer: w})
}
...
func RequestLiveTokenContext(ctx context.Context, h AuthCodeHandler) (*oauth2.Token, error) {
	return AndroidConfig.RequestLiveTokenContext(ctx, h)
}
...
// Once fully authenticated, an oauth2 token is returned which may be used to login to XBOX Live.
// The context is used to control the deadline for polling the OAuth2 device authorization endpoint.
func (conf Config) RequestLiveTokenContext(ctx context.Context, h AuthCodeHandler) (*oauth2.Token, error) {
	d, err := conf.DeviceAuth(ctx)
	if err != nil {
		return nil, fmt.Errorf("start device auth: %w", err)
	}

	h.DisplayCode(d.VerificationURI, d.UserCode)
...
type AuthCodeHandler interface {
	DisplayCode(VerificationURI, UserCode string)
	Successful()
}

type CliAuthCodeHandler struct {
	io.Writer
}

var _ AuthCodeHandler = (*CliAuthCodeHandler)(nil)

// DisplayCode implements [AuthCodeHandler].
func (c CliAuthCodeHandler) DisplayCode(VerificationURI string, UserCode string) {
	_, _ = fmt.Fprintf(c, "Authenticate at %v using the code %v.\n", VerificationURI, UserCode)
}

// Successful implements [AuthCodeHandler].
func (c CliAuthCodeHandler) Successful() {
	_, _ = c.Write([]byte("Authentication successful.\n"))
}
